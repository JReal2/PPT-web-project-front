...
mentoringID = localStorage.getItem('mentoringID'); // 아마 이 전 화면들 어딘가에서 localStorage에 setItem으로 서버에서 받아온 mentoring의 Id를 저장했을거임 그걸 꺼내쓰는것
accessToken = localStorage.getItem('accessToken'); //로그인하면 받는 accessToken 
userID = localStorage.getItem('userID');
mentorName = localStorage.getItem('mentorName');
menteeName = localStorage.getItem('menteeName');
...
var stompClient = Stomp.over(socket);
        ...
        function showMessage(message) {
            ...
            if (message.userId == userID) {
                ...
                imageElement.attr('src', 'person1.png');
            } else {
                messageElement.addClass('received');
                imageElement.attr('src', 'person3.png');
            }
...
function makeChatDiv(userId, message, createTime) {
    ...
    if(userId == userID) {
        img.setAttribute('src', 'person1.png');
        msgDiv.classList.add("sent");
    } else { //이런식으로 구분하면되지않을까함
        img.setAttribute('src', 'person3.png');
